...
import { getNextDueDate } from '../helper';
...
const DashboardCopy = () => {
   ...
   const handlePburchase = async (course) => {
      if (!user) {
         setShowAuthModal(true); // Open custom modal instead of alert
         return;
      }
      const totalPayable = course?.remainingBalance
      const amountPaid = ((course.remainingBalance) / (course?.totalInstallments - course?.currentInstallmentNumber)).toFixed(2)
      ...
      const payload = {
         ...
         paymentDetails: {
            sellingPrice: course?.pricing?.sellingPrice,
            mrp: course?.pricing?.mrp,
            discountAmount: course?.pricing?.discountAmount,
            amountPaid: amountPaid,
            internetHandlingCharge: 0,
            gst: 0,
            isCouponApplied: course?.isCouponApplied,
            couponCode: course?.isCouponApplied?.couponCode,
            isInstallment: course?.isInstallment,
            installment: course?.isInstallment
               ? {
                  isInstallment: true,
                  totalInstallments: course?.totalInstallments,
                  currentInstallmentNumber: course?.currentInstallmentNumber + 1,
                  amountPaid: amountPaid,
                  remainingBalance:
                     Math.max(totalPayable - amountPaid, 0),
                  duration: "1 month",
                  nextDueDate: getNextDueDate("1 month")
               }
               : null
         }
      };
      ...
      const options = {
         ...
         amount: Math.round(parseFloat(amountPaid) * 100), // Razorpay works in paise (multiply by 100)
         currency: "INR",
         name: "Bhavanam SC2C",
                           ...
                           <div className="p-6 space-y-4">
                              ...
                              {hasPendingInstallment ? (
                                 <div className="bg-orange-50 border border-orange-100 p-4 rounded-xl flex items-start gap-3">
                                    ...
                                    <button onClick={() => handlePburchase(course)} className="text-[10px] bg-orange-600 text-white px-3 py-1.5 rounded-lg font-bold hover:bg-orange-700">
                                       Pay ₹{((course.remainingBalance) / (course?.totalInstallments - course?.currentInstallmentNumber)).toFixed(2)}
                                    </button>
                                 </div>
                              ) : (
